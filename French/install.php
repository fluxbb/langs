...
'Info 4'				=>	'Le nom de la base de données dans laquelle FluxBB sera installé. La base de donnée doit exister. Pour SQLite, indiquer le chemin relatif vers le fichier de base de données. Si le fichier de base de données SQLite n\'existe pas, FluxBB tentera de le créer.',
...
'Maintenance message'			=>	'Les forums sont temporairement inaccessibles pour maintenance. Veuillez essayer à nouveau dans quelques minutes.<br />\\n<br />\\n/Administrateur',
...
'Alert cache'				=>	'<strong>Le répertoire cache n\'est pas modifiable actuellement !</strong> Pour que FluxBB fonctionne correctement, le répertoire nommé <em>cache</em> doit être modifiable par PHP. Utilisez chmod pour spécifier les permissions appropriées au répertoire. Si vous avez un doute, passez le chmod à 0777.',
'Alert avatar'				=>	'<strong>Le répertoire avatar n\'est pas modifiable actuellement !</strong> Si vous souhaitez que les utilisateurs puissent envoyer vers le serveur leur propre image avatar, vous devez vérifier que le répertoire nommé <em>img/avatars</em> soit modifiable par PHP. Vous pourrez plus tard choisir d\'enregistrer les images avatar dans un autre répertoire (rendez-vous à Admin/Options). Utilisez chmod pour spécifier les permissions appropriées au répertoire. Si vous avez un doute, passez le chmod à 0777.',
