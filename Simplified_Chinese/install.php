...
'Info 4'						=>	'安装 FluxBB 即将使用的数据库的名称。该数据库必须存在。如使用 SQLite 数据库，这里便是数据库文件的相对路径。如 SQLite 数据库文件不存在，FluxBB 将尝试创建它。',
'Database name'					=>	'数据库名',
...
'Rules'							=>	'在此填写论坛服务条款',
'Maintenance message'			=>	'论坛维护中。请稍后访问。<br />\\n<br />\\n/管理员',
'Test post'						=>	'示例帖子',
'Message'						=>	'如果您在阅读这个帖子(我猜正是如此)，安装的 FluxBB 已经开始工作！现在您可以登录，并进入顶部的管理设置面板来配置您的论坛。',
'Test category'					=>	'测试分类',
'Test forum'					=>	'测试版块',
'This is just a test forum'		=>	'这只是一个测试版块',
'Alert cache'					=>	'<strong>缓存所在目录不可写！</strong>为使 FluxBB 功能正常，必须为目录 <em>cache</em> 开启 PHP 写入权限。使用 chmod 命令设置恰当的目录权限。如有疑问，用 chmod 设为0777。',
'Alert avatar'					=>	'<strong>头像所在目录不可写！</strong>如果您准备允许用户上传自定义头像，您必须确认已为目录 <em>img/avatars</em> 开启了 PHP 写入权限。稍后您可为用户上传的头像文件选择指定的目录(见 管理设置/配置选项)。使用 chmod 命令设置恰当的目录权限。如有疑问，用 chmod 设为0777。',
