...
'Info 2'						=>	'FluxBB 目前支援 MySQL， PostgreSQL 以及 SQLite。如果選擇資料庫的下拉式選單不可用，便表示該 PHP 環境不支援所需的資料庫。各資料庫特定版本的更多相關資料可查看問答集(FAQ)。',
'Dual MySQL'					=>	'FluxBB 檢測到您的 PHP 環境支援 MySQl 的兩種不同通訊方式。兩個選項分別為標準(standard)與加強(improved)。如果您不確定該如何選擇，建議您選擇加強(improved)，如不能完成再改試標準(standard)。',
'InnoDB'						=>	'FluxBB 檢測到您的 MySQL 也許支援 <a href="http://dev.mysql.com/doc/refman/5.0/en/innodb.html">InnoDB</a>。它對 執行大型論壇有益處。如果您不很確定是否該使用 InnoDB，建議您不要使用。',
'Database type'					=>	'資料庫類型',
'Required'						=>	'(必需)',
...
'Info 4'						=>	'安裝 FluxBB 即將使用的資料庫名稱。該資料庫必須存在。如使用 SQLite 資料庫，這裡便是資料庫檔案的相對路徑。安全起見，強烈建議儘可能將 SQLite 資料庫放置於私有(網路不能存取)目錄。如 SQLite 資料庫檔案不存在，FluxBB 將嘗試建立它。',
'Database name'					=>	'資料庫名稱',
...
'Alert upload'					=>	'<strong>該伺服器似乎不允許上傳檔案！</strong>如果您準備允許會員上傳自訂頭像，您必須設定 PHP 中的 file_uploads 設定。上傳功能一旦啟用，可在 論壇管理設定/選項/頭像 中啟用。',
